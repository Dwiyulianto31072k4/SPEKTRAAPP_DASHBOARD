...
def name_clusters_fixed(n_clusters=4):
    ...
    segment_names = ["Potential Loyalists", "Responsive Customers", "Occasional Buyers", "Hibernating Customers"]
...
def generate_promo_recommendations(segmented_data, cluster_col='Cluster'):
    ...
    for _, row in cluster_metrics.iterrows():
        ...
        elif segment_name == "One-Time Buyers" or segment_name == "Responsive Customers":
...
def perform_optimal_segmentation(data, recency_col, frequency_col, monetary_col, reference_date=None):
    ...
    rfm['R_Score'] = pd.cut(
        rfm['Recency'], 
        bins=[0] + recency_quantiles + [float('inf')], 
        labels=[5, 4, 3, 2, 1],
        include_lowest=True
    ).astype('int')
    ...
    rfm['F_Score'] = pd.cut(
        rfm['Frequency'], 
        bins=[0] + frequency_quantiles + [float('inf')], 
        labels=[1, 2, 3, 4, 5],
        include_lowest=True
    ).astype('int')
    ...
    rfm['M_Score'] = pd.cut(
        rfm['Monetary'], 
        bins=[0] + monetary_quantiles + [float('inf')], 
        labels=[1, 2, 3, 4, 5],
        include_lowest=True
    ).astype('int')
